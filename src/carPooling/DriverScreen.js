...
export default class DriverScreen extends Component {
  ...
  _startMakingOrder = async () => {
    ...
    const currentPlace = deliveryFrom.split(',')[0].trim();
    const currentStreet = deliveryFrom.split(',')[1].trim();
    const currentCity = deliveryFrom.split(',')[2].trim();
    const currentCountry = deliveryFrom.split(',')[3].trim();

    const targetPlace = deliveryTo.split(',')[0].trim();
    const targetStreet = deliveryTo.split(',')[1].trim();
    const targetCity = deliveryTo.split(',')[2].trim();
    const targetCountry = deliveryTo.split(',')[3].trim();
    ...
    if(response){
        const { driver_id } = response
        await AsyncStorage.setItem("driver_id", JSON.stringify(driver_id));
